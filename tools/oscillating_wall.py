...
class OscillatingWallTool(BaseTool):
    name = "Oscillating Wall"
    icon = "water"

    placement_props = [
        {"key": "osc_sweep",    "label": "Sweep (deg)", "type": "float", "default": 90.0, "step": 5.0},
        {"key": "osc_speed",    "label": "Speed (Hz)",  "type": "float", "default": 0.5,  "min": 0.01, "step": 0.05},
    ...
    def on_mouse_release(self, world, pos, cam=None):
        if self.start_pos and self.current_pos:
            ...
            if length > 5:
                shapes = world.add_oscillating_wall(
                    self.start_pos, length,
                    sweep=self.defaults.get("osc_sweep", 90.0),
                    speed=self.defaults.get("osc_speed", 0.5),
                    rest_angle=0,
    ...
    def draw_preview(self, canvas, canvas_h, cam=None):
        if self.start_pos and self.current_pos:
            ...
            dpg.draw_line(p1, p2, color=(100, 200, 255, 150), thickness=4, parent=canvas)
            dpg.draw_circle(p1, 5, color=(100, 200, 255, 200), fill=(100, 200, 255, 100), parent=canvas)

    @staticmethod
    def get_properties(obj):
        import pymunk
        if not isinstance(obj, pymunk.Segment) or getattr(obj, "shape_type", "") != "OscillatingWall": return []
        return [
            {"key": "speed",       "label": "Speed (Hz)",   "type": "float", "value": getattr(obj, "speed", 0.5),    "min": 0.01, "step": 0.05},
            {"key": "sweep",       "label": "Sweep (deg)",  "type": "float", "value": getattr(obj, "sweep", 90.0),   "step": 5.0},
            {"key": "rest_angle",  "label": "Rest Angle",   "type": "float", "value": getattr(obj, "rest_angle", 0.0),"step": 5.0},
